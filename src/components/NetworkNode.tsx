  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="bg-linear-to-b from-[#2a2d3e] via-[#1a1c25] to-[#22242f] border-2 border-[#3d4251] rounded-xl relative flex flex-col shadow-[inset_0_2px_4px_rgba(255,255,255,0.07),inset_0_-2px_4px_rgba(0,0,0,0.5),0_10px_30px_rgba(0,0,0,0.8)] cursor-grab active:cursor-grabbing z-10 shrink-0 network-chassis pointer-events-auto"
        style={{ height: totalHeight, width: LAYOUT.EQUIP_WIDTH }}
      >
        ...
        {/* Header */}
        <div className="p-4 border-b border-white/5 flex items-center gap-3 h-[50px] shrink-0">
          ...
          <div className="flex flex-col min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <span className="text-[0.45rem] font-bold text-white/30 uppercase shrink-0">Type:</span>
              <input
                type="text"
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[0.45rem] font-bold text-white/30 uppercase shrink-0">Bldg:</span>
              <input
                type="text"
          ...
          <button 
            ...
            className="p-1 hover:bg-white/10 rounded border border-white/10 text-white/40 hover:text-[var(--accent)] transition-colors"
            title="Switch Side"
          >
        ...
        {/* Ports Labels */}
        <div className="flex-1 px-4 pt-2.5 flex flex-col gap-0 overflow-hidden">
          {Array.from({ length: equipment.ports }).map((_, i) => {
             ...
             return (
                <div key={i} className={`flex items-center gap-2 h-[30px] group/item ${isLeft ? 'flex-row-reverse' : 'flex-row'}`}>
                 <span className="text-[0.55rem] font-mono text-white/20 w-4 shrink-0">{(i + 1).toString().padStart(2, '0')}</span>
                 <div className="relative flex-1 group/input">
                    ...
                    />
                    <div className="absolute bottom-1 left-0 right-0 h-[1px] bg-white/5 group-hover/input:bg-[var(--accent)]/30 transition-colors" />
                 </div>
               </div>
             );
          })}
      ...
      <div className="relative">
        <svg 
          width={LAYOUT.FAN_GAP + 20} 
          height={totalHeight} 
          className="overflow-visible block"
        >
          {Array.from({ length: equipment.ports }).map((_, i) => {
            const portY = headerHeight + padding/2 + i * portHeight + portHeight/2;
            const dotX = isLeft ? 10 : LAYOUT.FAN_GAP;
            const startX = isLeft ? LAYOUT.FAN_GAP + 20 : 0; 
            ...
            return (
              <g key={`port-${i}`} className="group/port">
                ...
                <circle
                  ...
                  cy={portY}
                  r={LAYOUT.STRAND_R}
                  fill={isSelected ? "var(--accent)" : "#12151c"}
                  stroke={isSelected || isGlow ? "var(--accent)" : "rgba(255,255,255,0.3)"}
                ...
                <text
                  ...
                  className={`font-mono text-[7px] pointer-events-none transition-colors tracking-tight ${
                    isSelected ? 'fill-[var(--accent)] font-bold' : 'fill-white/30'
                  }`}
                >
                  P{i + 1}
                </text>
