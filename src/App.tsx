...
export default function App() {
  ...
  const getLocalProjectsMetadata = (): { id: string, name: string, updatedAt: string }[] => {
    try {
      const data = localStorage.getItem('fiberlink_projects_metadata_v2');
  ...
  const saveLocalProjectsMetadata = (metadata: { id: string, name: string, updatedAt: string }[]) => {
    localStorage.setItem('fiberlink_projects_metadata_v2', JSON.stringify(metadata));
  };

  const getLocalProjectDoc = (id: string): any | null => {
    try {
      const data = localStorage.getItem(`fiberlink_project_doc_${id}`);
  ...
  const saveLocalProjectDoc = (id: string, docData: any) => {
    localStorage.setItem(`fiberlink_project_doc_${id}`, JSON.stringify(docData));
  };

  const deleteLocalProjectDoc = (id: string) => {
    localStorage.removeItem(`fiberlink_project_doc_${id}`);
  };

  const loadLastLocalProject = () => {
    try {
      const rawMeta = getLocalProjectsMetadata();
      const projectList = rawMeta.map(p => ({
        id: p.id,
        name: p.name,
        updatedAt: {
      ...
      if (projectList.length > 0) {
        ...
        const sorted = [...rawMeta].sort((a,b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
        const latestId = sorted[0].id;
        const projectDoc = getLocalProjectDoc(latestId);
        if (projectDoc) {
          loadProjectData(latestId, projectDoc);
        }
      } else {
        // Initial setup for empty offline workspace
        setActiveProjectId('offline-default');
        ...
        saveLocalProjectDoc('offline-default', defaultDoc);
        saveLocalProjectsMetadata([{
          id: 'offline-default',
          name: 'Untitled Project',
          updatedAt: new Date().toISOString()
        }]);
      ...
      console.error('Error loading last local project:', err);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (errorCode === "auth/popup-blocked") {
        setAuthError("Popup blocked by the window. Please allow popups for this site, or try again.");
  ...
  if (authLoading) {
    return (
      <div className="h-screen w-screen bg-[#0a0d12] flex flex-col items-center justify-center font-sans relative overflow-hidden">
        {/* Background circuit grid */}
        <div className="absolute inset-0 futuristic-grid opacity-20 pointer-events-none" />
        <div className="relative z-10 flex flex-col items-center">
          <div className="w-16 h-16 border-4 border-[var(--accent)] border-t-transparent rounded-full animate-spin shadow-[0_0_15px_rgba(163,230,53,0.3)] mb-6" />
          <h2 className="text-xl font-bold text-white tracking-[6px] uppercase font-mono animate-pulse">
            Fiber Splice Workspace v3.0
          </h2>
          <span className="text-xs text-white/40 tracking-widest uppercase mt-2 font-mono">
            Loading Authenticated Environment...
          </span>
        </div>
      </div>
    );
  ...
  if (!user) {
    return (
      <div className="h-screen w-screen bg-[#0a0d12] flex items-center justify-center font-sans p-4 md:p-8 relative overflow-hidden">
        {/* Futuristic circuit grid and neon light sources */}
        <div className="absolute inset-0 futuristic-grid opacity-30 pointer-events-none" />
        <div className="absolute -top-[10%] -left-[10%] w-[50%] h-[50%] bg-[#a3e635]/5 rounded-full filter blur-[120px] pointer-events-none" />
        <div className="absolute -bottom-[10%] -right-[10%] w-[50%] h-[50%] bg-[#ff6a00]/5 rounded-full filter blur-[120px] pointer-events-none" />

        <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-12 gap-6 bg-[#0f1520]/80 border border-white/5 rounded-3xl overflow-hidden relative z-10 shadow-[0_0_50px_rgba(0,0,0,0.8)] backdrop-blur-2xl">
          {/* Left panel: Branding and system features schema */}
          <div className="md:col-span-12 lg:col-span-5 bg-[#141c2b]/50 p-8 md:p-12 border-r border-white/5 flex flex-col justify-between">
            <div className="flex flex-col gap-8">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-linear-to-br from-[#00d2ff] to-[#0088ff] rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(0,210,255,0.35)]">
                ...
                <div>
                  ...
                  <span className="text-[0.62rem] font-bold text-[var(--accent)] tracking-[3px] uppercase font-mono">
                    Workspace v3.0
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 mt-4">
                <h2 className="text-2xl font-bold text-white tracking-tight leading-snug font-sans">
                  The ultimate spatial toolkit for active network fiber optics mapping
                </h2>
                <p className="text-sm text-white/50 leading-relaxed font-sans">
                  Design detailed fiber splice connections, map physical network equipment, run instant visual laser trace analysis, and model zero-trust fiber infrastructure.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-4 mt-2">
                <div className="flex items-start gap-3">
                  <div className="mt-1 w-5 h-5 rounded-md bg-[var(--accent)]/10 border border-[var(--accent)]/30 flex items-center justify-center">
                    <Zap size={11} className="text-[var(--accent)]" />
                  </div>
                  <div>
                    <h3 className="text-xs font-bold text-white/80 uppercase tracking-wider font-mono">Visual Laser Fault Location</h3>
                    <p className="text-xs text-white/40 mt-0.5">Simulate laser fault testing and trace signals smoothly across active pathways.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-white/80 uppercase tracking-wider font-mono">Integrated Workspace</h3>
                    <p className="text-xs text-white/40 mt-0.5">Explore dual-axis optical splice diagrams and trace network circuits in real-time.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-white/80 uppercase tracking-wider font-mono">CAD, SVG & PDF Exports</h3>
                    <p className="text-xs text-white/40 mt-0.5">Instantly generate high-fidelity files ready for field deployment and printing.</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="text-[0.65rem] font-semibold text-white/30 font-mono tracking-wider mt-12">
              BUILD SECURITY INTEGRATION // USER ACCOUNT PORTAL
            </div>
          </div>

          {/* Right panel: Signup/Login panel */}
            ...
            <div className="max-w-md w-full mx-auto">
              <div className="flex rounded-xl bg-white/[0.03] border border-white/5 p-1 mb-8">
                <button
                  ...
                  className={`flex-1 text-center py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
                    isSignUpMode
                      ? "bg-[var(--accent)] text-[#0a0c12] shadow-[0_0_15px_rgba(163,230,53,0.3)] font-black"
              ...
              <div className="mb-6">
                ...
                <p className="text-sm text-white/50 mt-1 font-sans">
                  {isSignUpMode
                    ? "Verify your account and create optical splice schemas."
                    : "Access your saved design projects and optical diagrams."}
                </p>
              </div>
              ...
              <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4 mb-6">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[0.62rem] font-bold uppercase tracking-widest text-white/40 font-mono">Email Address</label>
                  <input
                    type="email"
                    required
                    value={authEmail}
                    onChange={(e) => setAuthEmail(e.target.value)}
                    placeholder="fieldengineer@broadband.com"
                    className="w-full h-11 px-4 rounded-xl bg-white/[0.04] border border-white/10 text-white text-sm focus:outline-none focus:border-[var(--accent)] focus:bg-white/[0.07] transition-all font-mono"
                  />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-[0.62rem] font-bold uppercase tracking-widest text-white/40 font-mono">Password</label>
                  <input
                    type="password"
                    required
                    value={authPassword}
              ...
              <div className="flex items-center my-4">
                ...
                <span className="px-3 text-[10px] text-white/30 uppercase tracking-widest font-mono select-none">or continue with</span>
                <div className="flex-1 border-t border-white/5"></div>
              </div>
              ...
              <button
                ...
                disabled={authSubmitting}
                className="w-full h-11 bg-white/[0.03] border border-white/10 hover:bg-white/[0.06] hover:border-white/20 active:scale-[0.98] rounded-xl text-white font-bold text-xs uppercase tracking-wider transition-all duration-200 cursor-pointer flex items-center justify-center gap-2.5 disabled:opacity-50 select-none shadow-[0_4px_20px_rgba(0,0,0,0.2)]"
              >
              ...
              <div className="relative font-mono text-[0.6rem] text-white/20 uppercase tracking-widest text-center mt-6">
  ...
  return (
    <div className={`h-screen w-screen relative flex flex-col overflow-hidden transition-colors duration-500 ${tool === 'analysis' ? 'bg-[#0a2fd1]' : 'bg-[var(--bg)]'} futuristic-grid`}>
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-[0.45rem] font-black text-[var(--accent)] tracking-[4px] uppercase leading-none mb-1">FIBER SPLICE WORKSPACE v3.0</span>
                <input 
                  type="text" 
                  value={projectName}
            ...
            <div className="flex items-center gap-1 group">
              ...
              <button onClick={() => saveToCloud(false)} className="icon-btn-refined px-3" title="Save Workspace">
                <Database size={14} className="text-[var(--accent)]" />
                <span className="text-[0.55rem] font-bold uppercase tracking-[2px] hidden lg:inline text-white/40 group-hover:text-white transition-colors">Save</span>
              </button>
            ...
            {user && (
              <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 px-3.5 py-1.5 rounded-xl">
                <div className="flex flex-col select-none text-right">
                  <span className="text-[0.62rem] text-emerald-400 font-bold uppercase tracking-wider font-mono">
                    Active User
                  </span>
                  <span className="text-[0.65rem] font-semibold text-white/80 font-mono max-w-[120px] truncate" title={user.email || ""}>
                    {user.email}
                  </span>
                </div>
      ...
      >
        ...
        >
          {tool === 'analysis' && (
            <div className="absolute inset-0 pointer-events-none bg-lime-500/5 animate-pulse" />
          )}
